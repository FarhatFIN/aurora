...
pub(crate) fn connect(url: &Url, cancel: &CancelToken) -> Result<Connection, NetError> {
        ...
        match TcpStream::connect_timeout(&addr, CONNECT_TIMEOUT) {
            Ok(stream) => {
                ...
                return if url.scheme() == "https" {
                    let tls = tls_handshake(stream, url, cancel)?;
...
fn tls_handshake(
    ...
    while tls.conn.is_handshaking() {
        ...
        // Drive the state machine; IO errors surface as TLS failures
        // (§5.3: verification failure is a hard TlsFailure, no click-through).
        match tls.conn.process_new_packets() {
            Ok(_) => {}
            Err(error) => return Err(NetError::Tls(tls_failure_text(&error))),
        }
        match std::io::Write::flush(&mut tls) {
            Ok(()) => {}
            Err(error) if error.kind() == std::io::ErrorKind::WouldBlock => {}
            Err(_) => return Err(NetError::Tls("handshake failed")),
        }
...
fn tls_failure_text(error: &rustls::Error) -> &'static str {
    match error {
        rustls::Error::InvalidCertificate(_) => "certificate verification failed",
...
/// Per-origin keep-alive connection pool (§5.2.1).
#[derive(Default)]
pub struct Pool {
